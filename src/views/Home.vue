...
export default {
  ...
  methods:{
    login(){
        ...
        axios.post('http://159.223.115.64:8000/api/auth/login', json)
        .then( data =>{
           if(data != null){
             localStorage.access_token = data.data.access_token;
             localStorage.token_type = data.data.token_type
             this.$router.push('dashboard');
           }else{
             this.error = true;
             this.error_msg = "Error al ingresar";
           }
        })
    }
  }
